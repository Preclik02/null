...
int main() {
	...
	else if (strcmp(command, "apt_update") == 0) {
		sys("sudo apt update");
	}
	else if (strcmp(command, "apt_install") == 0) {	
		char pacname[256];
		char command1[500];
		out("package name >> ");
		in("%255s", pacname);
		string(command1, sizeof(command1), "sudo apt install %s", pacname);
		sys(command1);
	}
