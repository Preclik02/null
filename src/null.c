...
void handle_sigint(int sig) {
    // Refresh the prompt cleanly on a new line when Ctrl+C is pressed at the prompt
    printf("\n[%s] %s > ", global_username, global_cwd);
    fflush(stdout);
}
...
int main() {
    ...
    if (access(path, F_OK) == 0) {
        FILE *file = fopen(path, "r");
        if (file) {
            fscanf(file, "commands_happened --> %d", &commands_happened);
            fclose(file);
        }
    }
    else {
    ...
    while (!stop) {
        ...
        if (strcmp(command, "quit") == 0) {
            ...
            printf("\n");
            signal(SIGINT, SIG_IGN); // Ignore signal in shell during system calls
            system("ls");
